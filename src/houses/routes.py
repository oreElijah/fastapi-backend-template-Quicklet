...
async def get_particular_house_by_uid(uid: str, session: AsyncSession= Depends(get_session), token: dict= Depends(AccessTokenBearer())):
    house = await house_service.get_house_by_id(uid, session)

    return house

@house_router.get("/{address}")
async def get_particular_house_by_address(address: str, session: AsyncSession= Depends(get_session), token: dict= Depends(AccessTokenBearer())):
    house = await house_service.get_house_by_address(address, session)

    return house

@house_router.post("/create")
async def create_house(house_model: HouseCreateModel, session: AsyncSession= Depends(get_session), token_details: dict= Depends(AccessTokenBearer()), _: bool= Depends(RoleChecker(["host", "admin"]))):
